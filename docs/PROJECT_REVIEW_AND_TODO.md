...
> - **Account management (mới)**: user tự đổi password (`/change-password`), tự re-enroll giọng (`/reenroll-voice`), tự bổ sung mẫu (`/add-voice-samples`); admin có masked-info + Reroll/Revoke voice + flow reset password có xác minh, chỉ cấp mã reset một lần chứ không đặt mật khẩu trực tiếp.
...
- [x] **Augmentation cho training** — Đã thêm khung pipeline trong `training/augmentations.py` và hook CLI ở `training/train_ecapa.py`: MUSAN noise, RIR reverb, SpecAugment, speed perturb. Cần chạy retrain trên Kaggle GPU để sinh artifact EER/minDCF mới; không impact runtime.
...
- [x] **Admin-assisted reset password** (P2-M) — user quên mật khẩu sẽ nhận mã qua Gmail đã link; nếu Gmail thiếu/hết hạn/lỗi thì tạo request pending cho admin. Admin Panel cho phép duyệt/từ chối request; khi duyệt phải chọn `verification_method` + ghi `admin_note`, endpoint `POST /api/admin/password-reset-requests/<id>/approve` sinh mã reset một lần (hash lưu DB, raw code chỉ trả về 1 response) để user tự đặt mật khẩu qua `/forgot-password/confirm`. Audit `admin.password_reset_request` + regression test.
