...
def main(args):
    ...
    # Fine-tune / resume from an existing checkpoint.
    # Use this when you already trained a speaker model, then want to
    # continue training with MUSAN/RIRS augmentation.
    if args.resume_ckpt:
        try:
        ...
        if "model" in ckpt:
            model.load_state_dict(ckpt["model"], strict=args.strict_resume)
            print(f"Loaded ECAPA model from: {args.resume_ckpt}")
        else:
            model.load_state_dict(ckpt, strict=args.strict_resume)
            print(f"Loaded raw model state_dict from: {args.resume_ckpt}")

        if args.resume_classifier and isinstance(ckpt, dict) and "classifier" in ckpt:
            try:
                classifier.load_state_dict(ckpt["classifier"], strict=args.strict_resume)
                print("Loaded classifier from checkpoint")
            except RuntimeError as e:
                print("WARNING: could not load classifier. This usually means the speaker set changed.")
                print(e)

        print(f"Resume checkpoint epoch={ckpt.get('epoch', '?') if isinstance(ckpt, dict) else '?'} "
              f"val_acc={ckpt.get('val_acc', '?') if isinstance(ckpt, dict) else '?'}")
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            torch.save(latest_payload, save_dir / "latest_model.pt")
            if args.save_every_epoch:
                torch.save(latest_payload, save_dir / f"epoch_{epoch:03d}.pt")
...
if __name__ == "__main__":
    ...
    p.add_argument("--resume-ckpt", default=None,
                   help="Path tới checkpoint .pt cũ để fine-tune tiếp")
    p.add_argument("--resume-classifier", action="store_true",
                   help="Load cả classifier từ checkpoint. Dùng khi cùng speaker set/split như lần train trước")
    p.add_argument("--strict-resume", action="store_true",
                   help="Strict load checkpoint. Mặc định strict=False để dễ fine-tune")
    p.add_argument("--save-every-epoch", action="store_true",
                   help="Save epoch_XXX.pt after every epoch so Kaggle runs can resume")
